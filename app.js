...
// 11. ABONELİK SİSTEMİ
// ==========================================
const subIconMap = { 'Elektrik': '<i class="fa-solid fa-bolt" style="color: #eab308;"></i>', 'Su': '<i class="fa-solid fa-droplet" style="color: #3b82f6;"></i>', 'Doğalgaz': '<i class="fa-solid fa-fire" style="color: #ef4444;"></i>', 'İnternet': '<i class="fa-solid fa-wifi" style="color: #8b5cf6;"></i>', 'Telefon': '<i class="fa-solid fa-phone" style="color: #10b981;"></i>', 'Diğer': '<i class="fa-solid fa-file-invoice" style="color: #94a3b8;"></i>' };

function renderTicker() {
    if(!tickerEl) return;
    if(subscriptions.length === 0) { tickerEl.innerHTML = '<span style="color: var(--text-muted);">Henüz abonelik eklenmedi. Sol menüden "Ayarlar"a giderek ekleyebilirsiniz.</span>'; return; }
    let html = ''; const displayList = [...subscriptions, ...subscriptions, ...subscriptions]; 
    displayList.forEach((sub) => { const icon = subIconMap[sub.category] || subIconMap['Diğer']; html += `<div class="ticker-item">${icon} <span>${sub.name}:</span> <span class="ticker-number">${sub.number}</span></div><i class="fa-solid fa-circle-small ticker-separator"></i>`; });
    tickerEl.innerHTML = html;
}

window.removeSubscription = function(id) { subscriptions = subscriptions.filter(s => s.id !== id); localStorage.setItem('subscriptions', JSON.stringify(subscriptions)); initSubscriptions(); if(accessToken) backupToDrive(true); };

function initSubscriptions() {
    if(subListEl) subListEl.innerHTML = '';
    subscriptions.forEach(sub => {
        const item = document.createElement('li'); item.classList.add('transaction-item'); const icon = subIconMap[sub.category] || subIconMap['Diğer'];
        item.innerHTML = `<div class="transaction-info"><span style="display:flex; align-items:center; gap:8px;">${icon} ${sub.name} <span class="category-badge">${sub.category}</span></span><small>Abone No: <strong>${sub.number}</strong></small></div><div><button class="list-btn delete-btn" onclick="removeSubscription(${sub.id})" title="Sil"><i class="fa-regular fa-trash-can"></i></button></div>`;
...
if(subForm) {
    subForm.onsubmit = (e) => {
        e.preventDefault();
        const cat = document.getElementById('sub-category').value; const name = document.getElementById('sub-name').value; const num = document.getElementById('sub-number').value;
        subscriptions.push({ id: Date.now(), category: cat, name: name, number: num });
        localStorage.setItem('subscriptions', JSON.stringify(subscriptions));
        initSubscriptions(); subForm.reset(); showNotify("Abonelik Eklendi!", "fa-check"); if(accessToken) backupToDrive(true);
    };
}
